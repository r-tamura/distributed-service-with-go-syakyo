...
func TestMembership(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		return len(handler.joins) == 2 && len(members[0].Members()) == 3 && len(handler.leaves) == 0
	}, 3*time.Second, 250*time.Millisecond)

	require.NoError(t, members[2].Leave())

	require.Eventually(t, func() bool {
		return len(handler.joins) == 2 && len(members[0].Members()) == 3 && members[0].Members()[2].Status == serf.StatusLeft && len(handler.leaves) == 1
	}, 3*time.Second, 250*time.Millisecond)
